...
<div id="sidebar-menu" class="main_menu_side hidden-print main_menu">
    <div class="menu_section">
        ...
        <ul class="nav side-menu">
            ...
            @if (Helper::authorizing('Order Refund', 'View List')['status'] == 'true')
                @php
                    $menu_active = '';
                    if(Helper::is_menu_active('/refund-order/')){
                        $menu_active = 'current-page';
                    }
                @endphp
                <li class="{{ $menu_active }}">
                    <a href="{{ route('admin.refund_order') }}">
                        <i class="fa fa-undo"></i> {{ ucwords(lang('refund Pesanan', $translations)) }}
                    </a>
                </li>
            @endif
            ...
            @if (Helper::authorizing('Order Report', 'View')['status'] == 'true')
                @php
                    $menu_active = '';
                    if(Helper::is_menu_active('/order-report/')){
                        $menu_active = 'current-page';
                    }
                @endphp
                <li class="{{ $menu_active }}">
                    <a href="{{ route('admin.order_report') }}">
                        <i class="fa fa-shopping-cart"></i> {{ ucwords(lang('Rekap Pesanan', $translations)) }}
                    </a>
                </li>
            @endif

            @if (Helper::authorizing('Order Invoice', 'View')['status'] == 'true')
                @php
                    $menu_active = '';
                    if(Helper::is_menu_active('/order-invoice/')){
                        $menu_active = 'current-page';
                    }
                @endphp
                <li class="{{ $menu_active }}">
                    <a href="{{ route('admin.order_invoice') }}">
                        <i class="fa fa-file-text"></i> {{ ucwords(lang('Rekap Pembayaran Pesanan', $translations)) }}
                    </a>
                </li>
            @endif

            @if (Helper::authorizing('Order Summary', 'View')['status'] == 'true')
                @php
                    $menu_active = '';
                    if(Helper::is_menu_active('/order-summary/')){
                        $menu_active = 'current-page';
                    }
                @endphp
                <li class="{{ $menu_active }}">
                    <a href="{{ route('admin.order_summary') }}">
                        <i class="fa fa-shopping-cart"></i> {{ ucwords(lang('Rekap Order Pesanan Terlaris', $translations)) }}
                    </a>
                </li>
            @endif

            @if (Helper::authorizing('Stock Recap', 'View')['status'] == 'true')
                @php
                    $menu_active = '';
                    if(Helper::is_menu_active('/stock_recap/')){
                        $menu_active = 'current-page';
                    }
                @endphp
                <li class="{{ $menu_active }}">
                    <a href="{{ route('admin.stock_recap') }}">
                        <i class="fa fa-area-chart"></i> {{ ucwords(lang('Product Stock Recap', $translations)) }}
                    </a>
                </li>
            @endif
        </ul>
