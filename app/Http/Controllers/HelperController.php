...
class HelperController extends Controller
{
    ...
    public function filter_sku_variant(Request $request)
    {
        $validation = [
            'parent' => 'required'
        ];
        ...
        $order = order::select('*')->where('transaction_id', $parent)->where('progress_status', '=', 3)->first();
        $order_id = (int) $order->id;

        // get postal codes from village
        $data = DB::table('order_details')
            ->select('product_item_variant.id', 'product_item_variant.sku_id')
