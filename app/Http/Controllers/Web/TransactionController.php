    ...
    public function create_transaction(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            try {
                ...
                $destination_code = $destination->zip_code;
                ...
                foreach ($total_weight_per_seller as $seller_id => $total_weight) {
                    ...
                    foreach ($check_tariff['data'] as $item) {
                        ...
                        $service_method = $item->code .' '. $item->service;

                        // get ongkos kirim per seller
                        // if ($selected_shipping_method == $item->product_code) {
                        if ($selected_shipping_method == $service_method) {
                            // $shipping_fee_per_seller = $item->rates;
                            $shipping_fee_per_seller = $item->cost;
                ...
                foreach ($subtotal_per_seller as $seller_id => $subtotal_seller) {
                    ...
                    $total_days = $shipping_per_seller_details['estimate_arrived_at'];
                    // $order->estimate_arrived_at = date('Y-m-d', strtotime($campaign_end_per_seller[$seller_id] . ' +' . $total_days . ' days'));
                    $order->estimate_arrived_at = date('Y-m-d', strtotime($total_days . ' days'));
                ...
                if (isset($payment_invoice['request'])) {
                    ...
                    $payment->request_data = json_encode($payment_invoice['request']);
                    $payment->response_data = json_encode($payment_invoice['response']);
                    $payment->save();
