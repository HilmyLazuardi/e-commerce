...
class OrderController extends Controller
{
        ...
        if (isset($data[0])) {
            ...
            foreach ($data_output as $key => $item) {
                ...
                switch ($item[0]->progress_status) {
                    ...
                    case 6:
                        $item[0]->status = 'Selesai';
                        $item[0]->label = 'finished_label';
                        break;
                }
                    ...
                    $html .= '<div class="history_top">';
                        ...
                        $html .= '<table>';
                            ...
                            $html .= '<tr>';
                                ...
                                if (in_array($item[0]->progress_status, [2, 3, 5, 6])) { // ORDER DETAIL
    ...
    public function order_detail($transaction_id)
    {
        ...
        $data               = order::select(
            ...
            ->whereIn('order.progress_status', [2, 3, 5, 6])
            ->get();
        ...
        $shipper_code = explode(' ', $data[0]->shipper_service_type);
        $shipper_code = $shipper_code[0];
        ...
        $data_order = [
            ...
            'shipping_number'      => $data[0]->shipping_number,
            'progress_status'      => $data[0]->progress_status,
            'shipper_code'         => $shipper_code, // ambil kode shipper dari nama shipper_service_type
        ];
    ...
    public function ajax_check_shipment_anteraja(Request $request)
    {
        ...
        // CHECK SELLER ADDRESS FOR GET DATA ORIGIN
        $seller = seller::where('seller.id', $seller_id)->first();
        ...
        $tariff_code = $destination->zip_code;
    ...
    public function order_tracking_log(Request $request)
    {
        $awb = $request->awb;
        $courier = $request->courier;

        $endpoint = env('RAJAONGKIR_BASE_PATH') . '/order/api/v1/orders/history-airway-bill';

        $access_key_id = env('RAJAONGKIR_ACCESS_KEY_ID');
        
        $client = new Client();

        $endpoint = 'https://api-sandbox.collaborator.komerce.id/order/api/v1/orders/history-airway-bill';
        
        $headers = [
            'x-api-key' => $access_key_id, // Ganti dengan API Key valid
            'accept'    => 'application/json',
        ];
        
        $query = http_build_query([
            'shipping'     => $courier, // contoh: 'jne'
            'airway_bill'  => $awb,     // contoh: '0637132400441624'
        ]);
        ...
        try {
            ...
            if (($body['meta']['code'] ?? 0) == 200) {
                $history = $body['data']['history'] ?? [];
        
                $logs = collect($history)->map(function ($item) {
                    return [
                        'date' => $item['date'],
            ...
            return response()->json(['logs' => [], 'error' => $body['meta']['message'] ?? 'Unknown error'], 400);
        } catch (\GuzzleHttp\Exception\ClientException $e) {
            // dd($e->getResponse()->getBody()->getContents()); // akan cetak error detail dari server
            return response()->json([
                'logs' => [],
                'error' => 'Client error: ' . $e->getResponse()->getBody()->getContents()
            ], 400);
        }
        ...
        //     $data = $response->json();
        //     $manifest = $data['rajaongkir']['result']['manifest'] ?? [];

        //     $logs = collect($manifest)->map(function ($item) {
        //         return [
        //             'date' => $item['manifest_date'] . ' ' . $item['manifest_time'],
    ...
    {
        $request->validate([
            'order_transaction' => 'required|string',
            'status' => 'required|string|in:refunded,finished',
        ]);

        $order = Order::where('transaction_id', $request->order_transaction)->first();
        if (!$order) {
            return redirect()->back()->with('error', 'Order transaction tidak ditemukan.');
        }

        if ($request->status === 'refunded') {
            $progress_status = 5; // refunded
        } elseif ($request->status === 'finished') {
            $progress_status = 6; // finished
        }
        ...
        // SAVE THE DATA
        if ($order->save()) {
            return redirect()->route('web.order.order_detail', $request->order_transaction)->with([
        ...
        // FAILED
        return redirect()->route('web.order.order_detail', $request->order_transaction)->with('error_profile', 'Oops, terjadi kesalahan, silahkan coba lagi.');
    }
    ...
    public function submit_refund(Request $request)
    {
        ...
        $validation = [
            ...
            'refund_photo' => 'required|image|max:2048',
            'refund_note' => 'required|max:1000'
        ];

        $message    = [
            'required' => ':attribute wajib diisi',
            'image' => ':attribute harus mengupload gambar',
            'max' => ':attribute maksimal ukuran gambar adalah 2MB',
        ];

        $names      = [
            'order_transaction_id' => 'Order Transaction',
            'refund_photo' => 'Foto Refund',
            'refund_note' => 'Keterangan Refund'
        ];

        $this->validate($request, $validation, $message, $names);

        $refund_note = Helper::validate_input_text($request->refund_note);
        if (!$refund_note) {
            return back()->withInput()->with('error_address_details', 'Keterangan refund tidak valid');
        }

        DB::beginTransaction();
        try {
            // CHECK BUYER DATA BY ID
            $order = Order::where('transaction_id', $request->order_transaction_id)->first();
    
            // if ($request->file('refund_photo')) {
            //     // UPLOAD IMAGE
            //     $dir_path           = 'uploads/buyer_photo_refund/';
            //     $image_file         = $request->file('refund_photo');
            ...
            //     // dd(lang($image['message']), $image, $request->file('refund_photo'));
            ...
            //     $order->refund_photo = $dir_path . $image['data'];
            // }
            if ($request->file('refund_photo')) {
                $dir_path           = 'uploads/buyer_photo_refund/';
                $image_file         = $request->file('refund_photo');
                ...
                if ($image['status'] != 'true') {
                    return back()
                        ->withInput()
                        ->with('error', lang('Something wrong with #item', $this->translations, ['#item' => ucwords(lang('refund photo', $this->translations))]));
                }
            ...
            $order->progress_status = 5;
            $order->refund_note     = $refund_note;
            $order->save();
            
            DB::commit();

            return redirect()->route('web.order.order_detail', $order->transaction_id)->with([
     ...
     * ORDER REFUND PAGE
     */
    public function refund_page(Request $request)
    {
        if (!Session::get('buyer')) {
            return redirect()->route('web.auth.login');
        }
        ...
        return view('web.order.refund', compact('navigation_menu', 'buyer_id'));
    }

    public function refund_data(Request $request)
    {
        // KEYWORD
        $keyword = '';
        if ($request->keyword) {
        ...
        // // FILTER
        // if ($filter != '') {
            $data = $data->whereIn('order.progress_status', [3,5]);
        // }
    ...
    public function refund_order_create(Request $request)
    {
        $data               = Session::get('buyer');
        $navigation_menu    = HelperWeb::get_nav_menu();
        $buyer_provider     = buyer_provider::where('user_id', Session::get('buyer')->id)->get();

        $order_transaction_id = order::select(
            'order.id',
            'order.transaction_id',
        )
            ->where('order.buyer_id', (int) Session::get('buyer')->id)
            ->whereIn('order.progress_status', [3,5])
            ->orderBy('order.created_at', 'desc')
            ->get();

        return view('web.order.refund_create', compact('data', 'navigation_menu', 'buyer_provider', 'order_transaction_id'));
    }

    public function refund_order_store(Request $request)
    {
        if (!Session::get('buyer')) {
            return redirect()->route('web.home');
        }
        ...
        $validation = [
            ...
            'refund_photo' => 'required|image|max:2048',
            'refund_notes' => 'required|max:2000'
        ];
        ...
        $names      = [
            ...
            'refund_photo' => 'Foto Refund',
            'refund_notes' => 'Keterangan Refund'
        ];

        $this->validate($request, $validation, $message, $names);

        $refund_notes = Helper::validate_input_text($request->refund_notes);
        if (!$refund_notes) {
            return back()->withInput()->with('error_refund_notes', 'Keterangan refund tidak valid');
        }
        ...
        try {
            ...
            if (!$order) {
                return back()->withInput()->with('error', 'Order tidak ditemukan.');
            }

            $order_refund = new order_refund();
            ...
            if ($request->file('refund_photo')) {
                ...
                if ($image['status'] != 'true') {
                    return back()
                        ->withInput()
                        ->with('error_refund_photo', lang('Something wrong with #item', $this->translations, ['#item' => ucwords(lang('refund photo', $this->translations))]));
                }

                // GET THE UPLOADED IMAGE RESULT
                $order_refund->refund_photo = $dir_path . $image['data'];
            }
            
            $order_refund->order_id = $order->id;
            $order_refund->product_id = $request->sku_variant;
            $order_refund->refund_notes = $refund_notes;
            $order_refund->save();
            ...
            return redirect()->route('web.order.refund')->with([
                'success' => 'Refund berhasil diajukan.',
                'header' => '<b>Hore!</b>'
            ]);
        } catch (\Exception $e) {
